...
export const registerMessageBusHandlers = (io: Server) => {
  realtimeBus.on('message:new', (message: any) => {
    const channelId = message?.channelId?._id?.toString?.() || message?.channelId?.toString?.() || message?.channelId;
    if (channelId) {
  ...
  realtimeBus.on('message:delete', (payload: any) => {
    const channelId = payload?.channelId?._id?.toString?.() || payload?.channelId?.toString?.() || payload?.channelId;
    if (channelId) {
...
export const handleMessageSocket = (_io: Server, socket: Socket) => {
  socket.on('message:new', async (payload, ack) => {
    try {
      ...
      socket.emit('socket:error', { event: 'message:new', message: error.message });
    }
  });
  ...
  socket.on('message:update', async (payload, ack) => {
    try {
      ...
      ack?.({ success: false, message: error.message });
      socket.emit('socket:error', { event: 'message:update', message: error.message });
    }
  });
  ...
  socket.on('message:delete', async (payload, ack) => {
    try {
      ...
      ack?.({ success: false, message: error.message });
      socket.emit('socket:error', { event: 'message:delete', message: error.message });
    }
  });
};
