...
export class ChannelDAO extends BaseDAO<IChannel> {
  ...
  async findBySlug(workspaceId: string, slug: string) {
    return this.model.findOne({ workspaceId, slug }).lean();
  }

  async update(channelId: string, updateData: Partial<IChannel>) {
    return this.model.findByIdAndUpdate(channelId, updateData, { new: true }).lean();
  }
  ...
  async archive(channelId: string) {
    return this.model
      .findByIdAndUpdate(channelId, { $set: { isArchived: true } }, { new: true })
      .lean();
  }
}
